...
function deleteDockerContainer(id: string) {
  try {
    exec(`docker stop ${id} && docker rm ${id}`);
  } catch (err) {}
}
...
async function startServer() {
  ...
  // 13.3 GET Dynamic Web Library Search via Steam Store Search
  app.get("/api/catalog/search", async (req, res) => {
    ...
    try {
      ...
      if (data && data.items) {
        const steamTemplates = data.items.map((item: any) => {
          ...
          return {
            ...
            icon: "🎮",
            description: `Live-Suchtreffer via Steam (App-ID: ${item.id}). Konfiguriert für automatisierte Installation über Docker-Brückennetzwerke.`,
            recommendedRam: 8192,
        ...
        return res.json(steamTemplates);
      }
    } catch (e) {
